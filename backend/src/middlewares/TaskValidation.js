...
const TaskValidation = async (req, res, next) => {
    ...
    if(!macaddress)
        return res.status(400).json({ error: 'macaddress não informado' });
    else if(!type)
        return res.status(400).json({ error: 'type não informado' });
    else if(!title)
        return res.status(400).json({ error: 'title não informado' });
    else if(!description)
        return res.status(400).json({ error: 'description não informado' });
    else if(!when)
        return res.status(400).json({ error: 'when não informado' });
